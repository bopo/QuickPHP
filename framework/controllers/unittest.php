...
/**
 * PHPUnit QuickPHP web based test runner
 *
 * @package    QuickPHP/UnitTest
...
class QuickPHP_UnitTest_Controller extends Template_Controller
{
	/**
	...
	public function before()
	{
		...
		if ( ! Unittest_tests::enabled())
		{
			...
			throw new QuickPHP_Request_Exception('Unable to find a route to match the URI: :uri',
		...
		$this->config = QuickPHP::config('unittest');
		...
		QuickPHP_View::set_global('xdebug_enabled', $this->xdebug_loaded);
		QuickPHP_View::set_global('cc_archive_enabled', $this->cc_archive_enabled);
	}
	...
	public function report()
	{
		...
		{
			throw new QuickPHP_Exception('Cannot generate report');
		}
		...
		$runner = new QuickPHP_Unittest_Runner($suite);
		...
		else
		{
			...
			if ( ! file_exists($path))
			{
				throw new QuickPHP_Exception('Report directory :dir does not exist', array(':dir' => $path));
			}

			if ( ! is_writable($path))
			{
				throw new QuickPHP_Exception('Script doesn\'t have permission to write to report dir :dir ', array(':dir' => $path));
			}
