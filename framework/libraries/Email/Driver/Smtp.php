...
// +----------------------------------------------------------------------+
/**
 * QucikPHP 图形处理 GraphicsMagick 驱动.
 *
 * $Id: Smtp.php 8320 2011-10-05 14:59:55Z bopo $
 *
 * @author BoPo <ibopo@126.com>
 ...
 * @package Image
 */
class QuickPHP_Email_Driver_Smtp extends QuickPHP_Email_Abstract
{
    ...
    public function __construct($config)
    {
        if (empty($config['directory']))
        {
            ...
            if ( ! is_file($path = exec('which gm')))
                throw new QuickPHP_Image_Exception('gmagick_not_found');
        ...
        if ( ! is_file(realpath($config['directory']).'/gm'.$this->ext))
            throw new QuickPHP_Image_Exception('gmagick_not_found', array('gm'.$this->ext));
